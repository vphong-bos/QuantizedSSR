...
from reference.projects.mmdet3d_plugin.SSR.modules.decoder import CustomMSDeformableAttention
from reference.projects.mmdet3d_plugin.SSR.modules.temporal_self_attention import TemporalSelfAttention
from reference.projects.mmdet3d_plugin.SSR.modules.spatial_cross_attention import MSDeformableAttention3D
...
class SSRPerceptionTransformer(BaseModule):
    ...
    def get_bev_features(
        ...
        # op.save_tensor(bev_queries, 'embeddings/bev_queries.pt')
        # op.save_tensor(bev_pos, 'embeddings/bev_pos.pt')
        ...
        if prev_bev is not None:
            # op.save_tensor(prev_bev, 'pts/prev_bev.pt')
            ...
            if self.rotate_prev_bev:
                ...
                # op.save_tensor(prev_bev, 'pts/prev_bev_rotated.pt')
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            # op.save_tensor(feat, 'pts/feat.pt')
            if self.use_cams_embeds:
                feat = feat + self.cams_embeds[:, None, None, :].to(feat.dtype)
                # op.save_tensor(self.cams_embeds[:, None, None, :].permute(1, 0, 2, 3).to(feat.dtype), "embeddings/cams_embeds.pt")
            feat = feat + self.level_embeds[None,
                                            None, lvl:lvl + 1, :].to(feat.dtype)
            # op.save_tensor(self.level_embeds[None, None, lvl:lvl + 1, :].to(feat.dtype), "embeddings/level_embeds.pt")
            # op.save_tensor(feat, 'pts/feat_1.pt')
        ...
        # op.save_tensor(feat_flatten, 'pts/feat_flatten.pt')
