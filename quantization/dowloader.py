...
def get_kaggle_credentials():
    """
    Try to load Cityscapes credentials from Kaggle Secrets.
    ...
    Returns:
        dict with username/password if found, else None
    """
    try:
    ...
    try:
        ...
        username = client.get_secret("CITYSCAPES_USERNAME")
        password = client.get_secret("CITYSCAPES_PASSWORD")
        ...
        # Secret not found or not running in Kaggle
        return None
...
def get_local_credentials():
    """
    Load credentials from local unencrypted file if it exists.
    """
    ...
    if os.path.isfile(credentials_file):
        with open(credentials_file, 'r') as f:
            return json.load(f), credentials_file

    return None, credentials_file


def prompt_for_credentials(credentials_file):
    """
    Prompt user interactively for credentials and optionally store them.
    """
    ...
    store_question = "Store credentials unencrypted in '{}' [y/N]: ".format(
        credentials_file
    )
...
def login():
    ...
    # 2) Then local credentials file
    if credentials is None:
        credentials, credentials_file = get_local_credentials()

    # 3) Finally interactive prompt
    if credentials is None:
        ...
        credentials = prompt_for_credentials(credentials_file)
    ...
    if r.status_code != 302:
        # delete local cached creds if they were bad
...
def get_available_packages(*, session):
    r = session.get(
        "https://www.cityscapes-dataset.com/downloads/?list",
        allow_redirects=False
    )
    r.raise_for_status()
    return r.json()
